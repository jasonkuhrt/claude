...
const parseTsgoOutput = (output: string): Diagnostic[] => {
...
const getDiagnostics = (filePath: string): Effect.Effect<Diagnostic[], Error> =>
  Effect.gen(function* () {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    const program = Effect.gen(function* () {
      ...
      const formatted = diagnostics.map(
        (d) => `${d.file}:${d.line}:${d.character} - ${d.code}: ${d.message}`
      ).join("\n")
